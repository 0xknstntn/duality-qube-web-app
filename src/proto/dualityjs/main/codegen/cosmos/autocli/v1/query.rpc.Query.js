...
var createRpcQueryHooks = function createRpcQueryHooks(rpc) {
  ...
  var useAppOptions = function useAppOptions(_ref) {
    var request = _ref.request,
      options = _ref.options;
    return (0, _reactQuery.useQuery)(["appOptionsQuery", request], function () {
      if (!queryService) throw new Error("Query Service not initialized");
      return queryService.appOptions(request);
    }, options);
  };
  return {
    /** AppOptions returns the autocli options for all of the modules in an app. */useAppOptions: useAppOptions
  };
};
