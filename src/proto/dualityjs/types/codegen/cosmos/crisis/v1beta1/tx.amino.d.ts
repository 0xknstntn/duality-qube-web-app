...
export declare const AminoConverter: {
    "/cosmos.crisis.v1beta1.MsgVerifyInvariant": {
        aminoType: string;
        toAmino: ({ sender, invariant_module_name, invariant_route }: MsgVerifyInvariant) => MsgVerifyInvariantAminoType["value"];
        fromAmino: ({ sender, invariant_module_name, invariant_route }: MsgVerifyInvariantAminoType["value"]) => MsgVerifyInvariant;
    };
    "/cosmos.crisis.v1beta1.MsgUpdateParams": {
        aminoType: string;
        toAmino: ({ authority, constant_fee }: MsgUpdateParams) => MsgUpdateParamsAminoType["value"];
