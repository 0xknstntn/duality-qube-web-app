...
import { QueryParamsResponse, QueryInflationResponse, QueryAnnualProvisionsResponse } from "./query";
export class LCDQueryClient {
  ...
  async inflation(_params = {}) {
    ...
    return QueryInflationResponse.fromJSON(await this.req.get(endpoint));
  }
  /* AnnualProvisions current minting annual provisions value. */
  async annualProvisions(_params = {}) {
    const endpoint = `cosmos/mint/v1beta1/annual_provisions`;
    return QueryAnnualProvisionsResponse.fromJSON(await this.req.get(endpoint));
  }
}