...
import { QueryCurrentPlanResponse, QueryAppliedPlanResponse, QueryUpgradedConsensusStateResponse, QueryModuleVersionsResponse, QueryAuthorityResponse } from "./query";
export class LCDQueryClient {
  ...
  async currentPlan(_params = {}) {
    ...
    return QueryCurrentPlanResponse.fromJSON(await this.req.get(endpoint));
  }
  /* AppliedPlan queries a previously applied upgrade plan by its name. */
  async appliedPlan(params) {
    const endpoint = `cosmos/upgrade/v1beta1/applied_plan/${params.name}`;
    return QueryAppliedPlanResponse.fromJSON(await this.req.get(endpoint));
  }
  ...
  async moduleVersions(params) {
    ...
    if (typeof (params === null || params === void 0 ? void 0 : params.module_name) !== "undefined") {
      options.params.module_name = params.module_name;
    }
    const endpoint = `cosmos/upgrade/v1beta1/module_versions`;
    return QueryModuleVersionsResponse.fromJSON(await this.req.get(endpoint, options));
  }
  ...
  async authority(_params = {}) {
    ...
    return QueryAuthorityResponse.fromJSON(await this.req.get(endpoint));
  }
}